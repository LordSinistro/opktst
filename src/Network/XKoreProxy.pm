...
sub serverDisconnect {
	...
	warning("Disconnecting from server nextIP: $self->{nextIp} nextPort: $self->{nextPort}\n", "xkoreProxy");
...
sub onClientData {
	...
	$msg = $self->{tokenizer}->slicePacket2($msg, \$additional_data); # slice packet if needed
...
sub checkServer {
	...
	if (!$self->serverAlive()) {
		...
		#
		$self->{lastIp} = $self->{nextIp};
		$self->{lastPort} = $self->{nextPort};
