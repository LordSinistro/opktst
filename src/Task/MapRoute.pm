...
sub setNpcTalk {
	...
	# If we have a specific NPC Steps, we should close the openkore
	if ($self->{mapSolution}[0]{steps} == 'BOOM') {
		# Format portal error
		my $exitMessage = sprintf(
			"MapRoute - NPC Steps is BOOM, closing openkore.\n%s\n",
			$self->{mapSolution}[0]{portal}
		);
		# Log portal error
		error $exitMessage, "map_route";
		# Disable AI
		Commands::run('ai off');
