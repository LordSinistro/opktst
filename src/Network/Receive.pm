...
sub parse_account_server_info {
	...
	} elsif ($args->{switch} eq '0C32' && $masterServer->{serverType} eq "ROla") {
		$server_info = {
			len => 165,
...
sub login_pin_code_request {
	...
	if ($args->{flag} ne 0 && ($config{XKore} eq "1" || $config{XKore} eq "4")) {
...
sub sync_request_ex {
	...
	return if ($config{XKore} eq 1 || $config{XKore} eq 4); # let the clien hanle this
