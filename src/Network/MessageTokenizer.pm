...
sub slicePacket2 {
	...
	my $real_length = $self->{rpackets}{$switch}{length};
	my $packet;
	# packet size is not variable
	if (($real_length > 0) && (length($data) >= ($real_length + 1))) {
		if (length($data) > $real_length) {
			$packet = substr($data, 0, $real_length);
			$$additional_data = substr($data, $real_length + 1); # sliced data
		} else {
			$packet = $data;
			$$additional_data = undef;
		}
	}
	# packet is at correct size?
	else { 
		$packet = $data;
		$$additional_data = undef;
		if (length($data) > 5) {
			my $packet_length = unpack("v", substr($data, 2, 2));
			if ($packet_length > 5 && length($data) > ($packet_length + 1)) {
				my $next_data = substr($data, $packet_length + 1);
				if (length($next_data) > 1) {
					$packet = substr($data, 0, $packet_length);
					$$additional_data = $next_data;
				}
